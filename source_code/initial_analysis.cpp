...
int main(int argc, char *argv[])
{
	...
	ofstream timing_data("timing_data_sequential.csv", ofstream::out);
	timing_data << "pixels,samples,bounces,time" << endl << endl;

	/*time_sum = 0;
	for (int t = 0; t < tests; t++)
	{
		cout << "Image " << t + 1 << " of " << tests << "\n";
		generate_image("image_sequential_r" + to_string(w * h) + "_s" + to_string(samps) + "_b4", w, h, samps);
		cout << "\n";
	}
	timing_data << w * h << "," << samps << ",4," << time_sum / tests << endl;*/

	// Resolution
	cout << "Resolutions\n";
	// Double the resolution at every step
	int t_w[5] = { 256, 256 * 2, 256 * 3, 256 * 4, 256 * 5 };
	int t_h[5] = { 187, 187 * 2, 187 * 3, 187 * 4, 187 * 5 };
	for (int r = 0; r < 5; r++)
	{
		cout << "   " << to_string(t_w[r] * t_h[r]) << "\n";
		time_sum = 0;
		for (int t = 0; t < tests; t++)
		{
			cout << "      Image " << t + 1 << " / " << tests << "\n";
			generate_image("image_sequential_r" + to_string(t_w[r] * t_h[r]) + "_s" + to_string(samps) + "_b4", t_w[r], t_h[r], samps);
		}
		timing_data << t_w[r] * t_h[r] << "," << samps << ",4," << time_sum / tests << endl;
	}
	...
	// Samples
	cout << "Samples\n";
	int t_samps[5] = { 100, 200, 300, 400, 500 };
	for (int s = 0; s < 5; s++)
	{
		cout << "   " << to_string(t_samps[s]) << "\n";
		time_sum = 0;
		for (int t = 0; t < tests; t++)
		{
			cout << "      Image " << t + 1 << " / " << tests << "\n";
			generate_image("image_sequential_r" + to_string(w * h) + "_s" + to_string(t_samps[s]) + "_b4", w, h, t_samps[s]);
		}
		timing_data << w * h << "," << t_samps[s] << ",4," << time_sum / tests << endl;
	}
